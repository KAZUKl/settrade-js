...
__export(index_exports, {
  Context: () => Context,
  Investor: () => Investor,
  InvestorDerivatives: () => InvestorDerivatives,
  InvestorEquity: () => InvestorEquity,
  MarketData: () => MarketData,
  MarketRep: () => MarketRep,
  MarketRepDerivatives: () => MarketRepDerivatives,
  MarketRepEquity: () => MarketRepEquity,
  RealtimeDataConnection: () => RealtimeDataConnection,
  createSignature: () => createSignature,
  getCurrentMilliTimestampStr: () => getCurrentMilliTimestampStr,
  moneyToFloat: () => moneyToFloat,
  parseProtoTimestamp: () => parseProtoTimestamp,
  syncNtpTimeDiff: () => syncNtpTimeDiff
});
...
var import_ntp_time = __toESM(require("ntp-time"), 1);
var { ec: EC } = import_elliptic.default;
var ec = new EC("p256");
function createSignature(secret, content) {
  ...
  const msgHash = import_node_crypto.default.createHash("sha256").update(content).digest();
  const signature = key.sign(msgHash);
  return signature.toDER("hex");
}
async function syncNtpTimeDiff() {
  try {
    const client = new import_ntp_time.default.Client("2.asia.pool.ntp.org", 123);
...
var RealtimeDataConnection = class {
  ...
  async connect() {
    ...
    return new Promise((resolve, reject) => {
      this.client = import_mqtt.default.connect(wssUrl, {
        protocol: "wss",
        ...
        createWebsocket: (url, protocols, opts) => {
          ...
          return new import_ws.default(url, protocols, {
            ...opts,
            headers: {
              ...opts.headers,
